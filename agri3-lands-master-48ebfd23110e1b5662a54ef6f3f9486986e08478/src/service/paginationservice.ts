...
export class PaginationService {
   ...
   */
  public static async generateQueryWithMobilePagination(
    originalQuery: string, orderField: string, orderType: string, hasBottomIndex: boolean,
    hasSize: boolean, bottomIndex: number, size: number
  ): Promise<QueryResult> {
    const isRequireNewestData: boolean = !hasBottomIndex || bottomIndex === -1;
    const whereSql: string = !isRequireNewestData ? `WHERE ${orderField} < ${bottomIndex}` : "";
    const nullPosition: string = orderType === "DESC" ? "LAST" : "FIRST";

    let processQuery: string = `
      WITH temp_table AS (${originalQuery}),
          filter_table AS (SELECT * FROM temp_table ${whereSql} ),
          rank_table AS (
            SELECT *, ROW_NUMBER() OVER (ORDER BY ${orderField} ${orderType} NULLS ${nullPosition}) AS row_num FROM filter_table
            )
      SELECT *, (
        SELECT count(1) FROM rank_table) as total_size FROM rank_table
    `;
   ...
   * @param size size
   */
  public static async generateQueryWithWebPagination(
    originalQuery: string, orderField: string, orderType: string, hasPage: boolean,
    hasSize: boolean, page: number, size: number
  ): Promise<QueryResult> {
    ...
    processQuery = `WITH temp_table AS (${processQuery}) SELECT *, (
      SELECT count(1) FROM temp_table
      ) AS total_size  FROM temp_table ORDER BY ${orderField} ${orderType}  NULLS ${nullPosition} `;
    if (hasPage && hasSize) {
